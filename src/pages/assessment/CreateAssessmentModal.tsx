...
interface CreateAssessmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (name: string, file: File | null, mode: AssessmentMode) => Promise<void>;
  mode: AssessmentMode;
  isLoading?: boolean;
...
export const CreateAssessmentModal: React.FC<CreateAssessmentModalProps> = ({
  ...
  const availableEnvironments = selectedEnvironment ? [selectedEnvironment] : [];
  ...
  const handleFileChange = (
    ...
    if (config.allowedExtensions.length > 0 && !config.allowedExtensions.includes(fileExtension || '')) {
      ...
      setFileError(`Unsupported file format. Please upload a ${extensionList} file.`);
      setSelectedFile(null);
      setFilename('');
      return;
    }
  ...
  const isFormValid = assessmentName.trim() && 
          ...
          >
            ...
            >
              ...
              {availableEnvironments.map((env) => (
                <FormSelectOption key={env.id} value={env.id} label={env.name} />
              ))}
            </FormSelect>
            {fileError && (
              <div style={{ 
